...
// Import input validation system
const input_validation_1 = require("../lib/input-validation");
const input_security_1 = require("../lib/input-security");
const FormValidator_1 = require("../components/FormValidator");
// State
let DEBUG = false;
let movieData = [];
let remaining = 0;
...
// Form validation
let nameFormValidator = null;
let searchFormValidator = null;
// Utility functions
...
const submitVote = (0, ErrorBoundary_1.withAsyncErrorBoundary)(async (card, vibe, seen) => {
    ...
    // Validate inputs
    const validationResults = input_validation_1.inputValidator.validateFields([
        {
            name: 'userName',
            value: userName,
    ...
    const vibeResult = validationResults.get('vibe');
    if (!userNameResult?.isValid) {
        error_handler_1.errorHandler.handleError(new Error('Invalid username'), { component: 'MoviePoll', action: 'SUBMIT_VOTE' }, userNameResult?.errors[0] || 'Please enter a valid username');
        return;
    }
    if (!movieIdResult?.isValid) {
        error_handler_1.errorHandler.handleError(new Error('Invalid movie ID'), { component: 'MoviePoll', action: 'SUBMIT_VOTE' }, movieIdResult?.errors[0] || 'Invalid movie selection');
        return;
    }
    if (!vibeResult?.isValid) {
        error_handler_1.errorHandler.handleError(new Error('Invalid vibe rating'), { component: 'MoviePoll', action: 'SUBMIT_VOTE' }, vibeResult?.errors[0] || 'Please select a valid vibe rating');
        return;
    }
    // Sanitize inputs
    const sanitizedUserName = input_security_1.inputSecurity.sanitizeText(userName || '');
    const sanitizedMovieId = movieIdResult.sanitizedValue || movieId;
    const sanitizedVibe = vibeResult.sanitizedValue || vibe;
    try {
        ...
        const appError = error_handler_1.errorHandler.handleError(error, {
            component: 'MoviePoll',
            action: 'SUBMIT_VOTE',
            additionalData: { movieId: sanitizedMovieId, vibe: sanitizedVibe, seen, userName: sanitizedUserName }
...
function handleSearchInput(event) {
    ...
    const validationResult = input_validation_1.inputValidator.validateField({
        name: 'search',
        value: query,
    ...
    const sanitizedQuery = input_security_1.inputSecurity.sanitizeText(validationResult.sanitizedValue || query);
...
// Name entry form handling with validation
function setupNameEntryForm() {
    ...
    nameFormValidator = (0, FormValidator_1.createFormValidator)(form, {
        showErrors: true,
    ...
    nameFormValidator.addField(FormValidator_1.FormValidationHelpers.createUsernameField(usernameInput));
    ...
    function handleStartPoll() {
        ...
        if (!name)
            return;
        // Sanitize username
        const sanitizedName = input_security_1.inputSecurity.sanitizeText(name);
